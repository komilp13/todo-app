...
public class AssignLabelHandler
{
    private readonly ApplicationDbContext _dbContext;

    public AssignLabelHandler(ApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<TaskItemDto?> Handle(AssignLabelCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var alreadyAssigned = await _dbContext.TaskLabels
            .AnyAsync(tl => tl.TaskId == command.TaskId && tl.LabelId == command.LabelId, cancellationToken);

        if (!alreadyAssigned)
        {
            _dbContext.TaskLabels.Add(TaskLabel.Create(command.TaskId, command.LabelId));
