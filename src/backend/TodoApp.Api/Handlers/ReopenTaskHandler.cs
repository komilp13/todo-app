...
/// <summary>
/// Handler for ReopenTaskCommand that reopens a completed task back to Open status.
/// </summary>
public class ReopenTaskHandler
{
    private readonly ApplicationDbContext _dbContext;

    public ReopenTaskHandler(ApplicationDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    /// <summary>
    /// Reopens a completed task: sets Open status, unarchives it, clears completion time.
    /// Retains the original system list and places task at top (sortOrder = 0).
    /// Idempotent - calling multiple times is safe.
    /// Returns the updated task with full details; returns null if task not found or belongs to different user.
    /// </summary>
    public async Task<TaskItemDto?> Handle(ReopenTaskCommand command, CancellationToken cancellationToken = default)
    {
